  ...
  useEffect(() => {
    const fetchCategoryProducts = async () => {
      try {
        ...
        console.log('Category:', categoryRes.data.name);
        console.log('Products found:', prodData.length);
        console.log('Products:', prodData.map(p => ({ id: p.id, name: p.name, category: p.category })));
      } catch (err) {
    ...
    <div className="category-products-page">
      <div className="category-header">
        <div className="category-header-content">
          ...
          <p>Khám phá bộ sưu tập {category?.name?.toLowerCase()} với {products.length} sản phẩm</p>
        </div>
      </div>
