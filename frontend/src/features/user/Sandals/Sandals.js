...
const Sandals = () => {
  ...
  useEffect(() => {
    const fetchSandalsProducts = async () => {
      try {
        setLoading(true);
  ...
  return (
    <div className="sandals-page">
      <div className="sandals-header">
        <div className="sandals-header-content">
          ...
          <h1>Dép</h1>
          <p>Khám phá bộ sưu tập dép với {products.length} sản phẩm</p>
        </div>
      </div>

      <div className="sandals-products-container">
        {products.length > 0 ? (
          ...
          <div className="no-products">
            ...
            <p>Hiện tại chưa có sản phẩm dép.</p>
          </div>
        )}
      </div>
    </div>
